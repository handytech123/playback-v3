...
public:
    ...
    juce::Result arm(const juce::File& manifestFile, int songIndex) {
        ...
        int stemIndex=0;for (const auto& stem : *stems.getArray()) {
            ...
            const auto destination=stemIndex<=static_cast<int>(stemOutputs.size())?stemOutputs[stemIndex-1]:stemIndex-1,width=stemIndex<=static_cast<int>(stemChannels.size())?stemChannels[stemIndex-1]:1;auto stemGain=std::make_unique<GainRampAudioSource>(*transport);auto stemRoute=std::make_unique<RoutedAudioSource>(*stemGain,2,destination,width);stemRoute->setIemFirstDestination(iemOutput);stemRoute->setIemDestinationChannels(1);stemRoute->setIemGain(0.125f);stemRoute->setIemEnabled(true);mixer.addInputSource(stemRoute.get(),false);
    ...
    bool setMixerChannel(int index,float gain,bool muted,bool solo,bool iem){const auto count=static_cast<int>(stemGains.size())+3;if(index<0||index>=count)return false;gain=juce::jlimit(0.0f,maxMixerFader,gain);if(index<static_cast<int>(stemGains.size())){stemFaders[index]=gain;stemMuted[index]=muted;stemSolo[index]=solo;stemIem[index]=!muted;stemRoutes[index]->setIemEnabled(!muted);}else{const auto aux=index-static_cast<int>(stemGains.size());auxFaders[aux]=gain;auxMuted[aux]=muted;auxSolo[aux]=solo;auxIem[aux]=iem;if(aux==0)clickRoute.setIemEnabled(iem);else if(aux==1)cueRoute.setIemEnabled(iem);else if(padRoute)padRoute->setIemEnabled(iem);}applyStemMix(0.03);applyAuxMix();return true;}
