...
struct SongDeck {
    ...
    std::vector<float> stemFaders,stemTrims;
    ...
    void reset(){
        ...
        stemRoutes.clear();stemGains.clear();stemFaders.clear();stemTrims.clear();stemMuted.clear();stemSolo.clear();stemIem.clear();
...
public:
    ...
    juce::Result arm(const juce::File& manifestFile, int songIndex) {
        ...
        int stemIndex=0;for (const auto& stem : *stems.getArray()) {
            ...
            stemFaders.push_back(1.0f);stemTrims.push_back(1.0f);stemMuted.push_back(false);stemSolo.push_back(false);stemIem.push_back(true);stemGains.push_back(std::move(stemGain));stemRoutes.push_back(std::move(stemRoute));
    ...
    bool setStemTrim(int index,float gain,bool muted,bool solo,bool iem){gain=juce::jlimit(0.0f,maxMixerFader,gain);if(dualDeckMode){auto& deck=*activeDeck;if(index<0||index>=static_cast<int>(deck.stemGains.size()))return false;deck.stemTrims[index]=gain;deck.stemMuted[index]=muted;deck.stemSolo[index]=solo;deck.stemIem[index]=iem;deck.stemRoutes[index]->setIemEnabled(iem);applyDeckStemMix(deck,0.03);return true;}if(index<0||index>=static_cast<int>(stemGains.size()))return false;stemTrims[index]=gain;stemMuted[index]=muted;stemSolo[index]=solo;stemIem[index]=iem;stemRoutes[index]->setIemEnabled(iem);applyStemMix(0.03);return true;}
...
#include "DualDeckEngine.inc"
    ...
    void applyStemMix(double rampSeconds){const auto anySolo=anyMixerSolo();for(size_t index=0;index<stemGains.size();++index){const auto audible=musicEnabled&&!stemMuted[index]&&(!anySolo||stemSolo[index]);const auto trim=mixerFaderToGain(stemTrims[index]);stemRoutes[index]->setIemGain(playback::iem::stemHeadroomGain*normalizationGain*panicAttenuation*(musicEnabled?1.0f:0.0f));stemRoutes[index]->setExternalFactor(audible?trim*externalStemRatio(index,stemRoutes,stemFaders)*mixerFaderToGain(musicBusGain)*normalizationGain*panicAttenuation:0.0f);stemGains[index]->setGain(audible?trim*mixerFaderToGain(stemFaders[index])*mixerFaderToGain(musicBusGain)*normalizationGain*panicAttenuation:0.0f,rampSeconds);}}
    ...
    std::vector<std::unique_ptr<GainRampAudioSource>> stemGains;
    std::vector<std::unique_ptr<RoutedAudioSource>> stemRoutes;
    std::vector<float> stemFaders,stemTrims;std::vector<bool> stemMuted,stemSolo,stemIem;
...
int main(int argc, char* argv[]) {
    ...
    for (std::string command; std::cin >> command;) {
        ...
        else if(command=="stem_trim"){int index=0,muted=0,solo=0,iem=0;float value=1;std::cin>>index>>value>>muted>>solo>>iem;if(engine.setStemTrim(index,value,muted!=0,solo!=0,iem!=0))std::cout<<"STEM_TRIM index="<<index<<" value="<<value<<" muted="<<muted<<" solo="<<solo<<" iem="<<iem<<"\n";else std::cout<<"STEM_TRIM_FAILED index="<<index<<"\n";}
