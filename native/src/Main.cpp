...
public:
    ...
    bool setMixerChannel(int index,float gain,bool muted,bool solo,bool iem){const auto count=static_cast<int>(stemGains.size())+3;if(index<0||index>=count)return false;gain=juce::jlimit(0.0f,maxMixerFader,gain);if(index<static_cast<int>(stemGains.size())){const auto sendEnabled=playback::iem::stemSendEnabled(muted);stemFaders[index]=gain;stemMuted[index]=muted;stemSolo[index]=solo;stemIem[index]=sendEnabled;stemRoutes[index]->setIemEnabled(sendEnabled);}else{const auto aux=index-static_cast<int>(stemGains.size());auxFaders[aux]=gain;auxMuted[aux]=muted;auxSolo[aux]=solo;auxIem[aux]=iem;if(aux==0)clickRoute.setIemEnabled(iem);else if(aux==1)cueRoute.setIemEnabled(iem);else if(padRoute)padRoute->setIemEnabled(iem);}applyStemMix(0.03);applyAuxMix();return true;}
