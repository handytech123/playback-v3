...
function mount(header){
 ...
 <p style="color:#ffd18a">GLD-only mode removes the mapped bus gain from Playback audio. IEM sends remain pre-fader. Stop playback before setup. Surface Mixer ON arms recall automatically; OFF holds console levels. Neither switching nor a MIDI send failure stops playback. Shared GLD returns change at the start of a song transition, including overlaps.</p>
 <div style="display:flex;gap:12px;flex-wrap:wrap"><label>Connection <select id="gbTransport"><option value="midi">MIDI cable</option><option value="tcp">Network TCP</option></select></label><label>MIDI output <select id="gbMidi"></select></label><label>MIDI channel <input id="gbChannel" type="number" min="1" max="16" style="width:60px"></label><label>GLD IP <input id="gbHost" style="width:140px"></label><label>TCP port <input id="gbPort" type="number" value="51325" style="width:80px"></label></div>
 ...
 function render(state){status.textContent=state.status;$('gbStatus').textContent=(state.surfaceEnabled?'SURFACE MIXER ON · ':'SURFACE MIXER OFF · ')+state.status;}
