...
export async function saveArrangementVersion(root:string,arrangement:ArrangementVersion):Promise<string>{
  ...
  try{const existing=JSON.parse(await readFile(path,"utf8")) as ArrangementVersion;if(existing.sourceSha256===arrangement.sourceSha256){if(JSON.stringify(existing)!==JSON.stringify(arrangement)){const temporary=`${path}.tmp-${process.pid}-${Date.now()}`;await writeFile(temporary,JSON.stringify(arrangement,null,2),{encoding:"utf8",flag:"wx"});await rename(temporary,path);}return path;}throw new Error("Arrangement version id collision");}catch(error){if((error as NodeJS.ErrnoException).code!=="ENOENT")throw error;}
