...
export async function importReaperProject(path:string,songId:SongId,original?:PreparedSong):Promise<ArrangementImportPreview>{
  ...
  let regions:Region[]=rawRegions.map((region)=>({id:region.id,name:region.name,startPosition:locate(region.startSeconds),endPosition:locate(region.endSeconds),startSeconds:region.startSeconds,endSeconds:region.endSeconds}));
  ...
  let cueMarkers:ArrangementCue[]=markers.map((marker)=>cueFromMarker(marker,regions,regionByIndex,locate)).filter((cue):cue is ArrangementCue=>cue!==null);
  if(!cueMarkers.length&&regions[0])cueMarkers=[{phrase:"Intro",position:locate(0),atSeconds:0,targetRegionId:regions[0].id}];
  const tracks=children(project,"TRACK"),namedSlides=tracks.find((track)=>(value(track,"NAME")?.join(" ")??"").trim().toLowerCase()==="slides"),slides=namedSlides??tracks.find(track=>isProPresenterMidiItemTrack(track,bpm));
  ...
  if(inferredStarts.length){rawRegions=[...rawRegions,...inferredStarts.map((item,index)=>({id:`reaper-region-${String(rawRegions.length+index+1).padStart(4,"0")}`,name:item.marker.name,startSeconds:item.startSeconds,endSeconds:inferredStarts[index+1]?.startSeconds??durationSeconds}))];regions=rawRegions.map(region=>({id:region.id,name:region.name,startSeconds:region.startSeconds,endSeconds:region.endSeconds}));const indexed=[...regionStarts.map(start=>start.index),...inferredStarts.map(item=>item.marker.index)];regionByIndex=new Map(indexed.map((index,position)=>[index,regions[position]!]));cueMarkers=markers.map(marker=>cueFromMarker(marker,regions,regionByIndex,locate)).filter((cue):cue is ArrangementCue=>cue!==null);}
  ...
  const arrangement:ArrangementVersion={schemaVersion:1,id:`reaper-${hash.slice(0,12)}`,songId,name:reaperTitle(identity.name),sourceType:"reaper-import",sourcePath:path,sourceSha256:hash,importedAt:new Date().toISOString(),selectedKey:identity.key??original?.selectedKey??null,selectedBpm:bpm,timeSignature:{numerator,denominator},durationSeconds,regions,cueMarkers,markers,mediaItems,proPresenterMidi,slidesTrackName:slides?(namedSlides?"Slides":midiItemLabel(slides)):null,warnings};
...
function cueFromMarker(marker:ReaperMarker,regions:Region[],regionByIndex:Map<number,Region>,locate:(seconds:number)=>ReturnType<typeof secondsToMusicalPosition>):ArrangementCue|null{if(!marker.name)return null;const target=matchRegionForCue(marker,regions,regionByIndex);if(!target)return null;return{phrase:marker.name,position:locate(marker.atSeconds),atSeconds:marker.atSeconds,targetRegionId:target.id};}
function matchRegionForCue(marker:ReaperMarker,regions:Region[],regionByIndex:Map<number,Region>){const name=cueMatchKey(marker.name),byName=regions.find(region=>cueMatchKey(region.name)===name&&region.startSeconds>=marker.atSeconds-.05);return byName??regionByIndex.get(marker.index)??regions.find(region=>region.startSeconds>=marker.atSeconds-.05)??null;}
function cueMatchKey(value:string){return value.toLowerCase().replace(/[^a-z0-9]+/g,"");}
