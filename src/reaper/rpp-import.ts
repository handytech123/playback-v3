...
export async function importReaperProject(path:string,songId:SongId,original?:PreparedSong):Promise<ArrangementImportPreview>{
  ...
  const rawRegions=regionStarts.map((start,ordinal)=>{const end=regionEnds.find((item)=>item.index===start.index&&item.atSeconds>start.atSeconds);if(!end)throw new Error(`Reaper region ${start.name} has no end`);return{id:`reaper-region-${String(ordinal+1).padStart(4,"0")}`,name:start.name,startSeconds:start.atSeconds,endSeconds:end.atSeconds};});
  const normalized=normalizeRegions(rawRegions);
  const regions:Region[]=normalized.map((region)=>({id:region.id,name:region.name,startSeconds:region.startSeconds,endSeconds:region.endSeconds}));
  const regionByIndex=new Map(regionStarts.map((start,index)=>[start.index,regions[index]!]));
  ...
  const cueMarkers:ArrangementCue[]=markers.filter((marker)=>marker.name&&regionByIndex.has(marker.index)).map((marker)=>({phrase:regionByIndex.get(marker.index)!.name,atSeconds:marker.atSeconds,targetRegionId:regionByIndex.get(marker.index)!.id}));
  const tracks=children(project,"TRACK"),slides=tracks.find((track)=>(value(track,"NAME")?.join(" ")??"").trim().toLowerCase()==="slides");
  const mediaItems:ArrangementMediaItem[]=[];for(const track of tracks){const trackName=(value(track,"NAME")?.join(" ")??"").trim();for(const item of children(track,"ITEM")){const source=item.children.find((child)=>child.tag==="SOURCE");if(source?.args[0]==="MIDI")continue;const rawSource=value(source??emptyNode(),"FILE")?.join(" ")??null,sourcePath=rawSource?(isAbsolute(rawSource)?rawSource:resolve(dirname(path),rawSource)):null;mediaItems.push({trackName,positionSeconds:num(value(item,"POSITION")?.[0],0),lengthSeconds:num(value(item,"LENGTH")?.[0],0),sourcePath,sourceOffsetSeconds:num(value(item,"SOFFS")?.[0],0),playRate:num(value(item,"PLAYRATE")?.[0],1)});}}
  ...
  const arrangement:ArrangementVersion={schemaVersion:1,id:`reaper-${hash.slice(0,12)}`,songId,name:identity.name,sourceType:"reaper-import",sourcePath:path,sourceSha256:hash,importedAt:new Date().toISOString(),selectedKey:identity.key,selectedBpm:bpm,timeSignature:{numerator,denominator},durationSeconds,regions,cueMarkers,markers,mediaItems,proPresenterMidi,slidesTrackName:slides?"Slides":null,warnings};
...
function parseMarker(parts:string[]):ParsedMarker|null{const index=Number(parts[1]),atSeconds=Number(parts[2]);if(!Number.isFinite(index)||!Number.isFinite(atSeconds))return null;return{index,atSeconds,name:parts[3]??"",isRegion:parts[4]==="1"};}
...
function inferArrangementIdentity(path:string,bpm:number){const folder=basename(dirname(path)),file=basename(path).replace(/\.rpp$/i,"");const candidate=file||folder;const match=candidate.match(/(?:^|\s)([A-G](?:#|b)?)(?:\s|$)/i);return{name:candidate,key:match?.[1]??null,bpm};}
