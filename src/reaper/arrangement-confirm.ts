...
export async function confirmArrangement(input: ArrangementConfirmInput): Promise<{manifestPath:string;manifest:ConfirmedSetManifest}> {
  ...
  const cues=[];for(const cue of input.arrangement.cueMarkers){const phrase=spokenCuePhrase(cue.phrase),output=join(cueOutput,`${safe(phrase)}.wav`),sourcePath=await resolveCueSource(input.cueDirectory,phrase);await writeCountedCue({sourcePath,destinationPath:output,numberDirectory:input.cueDirectory,bpm:input.arrangement.selectedBpm,meter:input.arrangement.timeSignature,ffmpegPath:input.ffmpegPath??"ffmpeg"});cues.push({atSeconds:cue.atSeconds,label:phrase,audioPath:output,targetRegionId:cue.targetRegionId});}
