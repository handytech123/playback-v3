...
async function resolveCueSource(directory:string,label:string){const normalized=normalizeCueFileLabel(label),aliases:Record<string,string>={START:"CountIn.wav","COUNT OFF":"CountIn.wav","A CAPELLA":"ACAPPELLA.wav",ACAPELLA:"ACAPPELLA.wav"},names=[aliases[normalized.toUpperCase()]??`${normalized.toUpperCase()}.wav`,`${normalized.toUpperCase().replace(/\s+/g,"")}.wav`];for(const name of names){const candidate=join(directory,name);if(await exists(candidate))return candidate;}throw new Error(`No cue audio for ${label}`);}
function normalizeCueFileLabel(label:string){return label.trim().replace(/([a-z])([A-Z])/g,"$1 $2").replace(/([A-Za-z])([0-9])$/,"$1 $2").replace(/^Turn\s*Arround/i,"Turn Around").replace(/^Turnaround/i,"Turn Around").replace(/-/g," ").replace(/\s+/g," ");}
