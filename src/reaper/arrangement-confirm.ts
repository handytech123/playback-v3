...
export async function confirmArrangement(input: ArrangementConfirmInput): Promise<{manifestPath:string;manifest:ConfirmedSetManifest}> {
  ...
  const assets=join(input.outputDirectory,"live-assets"),cueOutput=join(assets,"cues");await mkdir(cueOutput,{recursive:true});
  ...
  await Promise.all(copies.map(async([source,destination])=>{if(!(await exists(destination)))await copyFile(source,destination);}));
  const cues=[];for(const cue of input.arrangement.cueMarkers){const output=join(cueOutput,`${safe(cue.phrase)}.wav`);if(!(await exists(output)))await buildCue(input.cueDirectory,cue.phrase,output,input.ffmpegPath??"ffmpeg");cues.push({atSeconds:cue.atSeconds,label:cue.phrase,audioPath:output,targetRegionId:cue.targetRegionId});}
  ...
  const fingerprint=createHash("sha256").update(input.arrangement.sourceSha256).update(JSON.stringify(input.stems.map(x=>[x.role,x.sourcePath]))).digest("hex");
  const manifest:ConfirmedSetManifest={schemaVersion:1,id:`confirmed-${input.arrangement.id}`,name:input.arrangement.name,confirmedAt:new Date().toISOString(),songs:[{song:input.originalSong,selectedKey:input.arrangement.selectedKey,selectedBpm:input.arrangement.selectedBpm,timeSignature:input.arrangement.timeSignature,durationSeconds:input.arrangement.durationSeconds,stems:input.stems.map(({role,sourcePath,durationSeconds})=>({role,sourcePath,durationSeconds})),regions:input.arrangement.regions,cues:input.arrangement.cueMarkers,cacheFingerprint:`sha256:${fingerprint}`,waveformPath,liveAssets:{click:{regularPath:regular,accentPath:accent,events:buildZeroBasedGrid(input.arrangement.selectedBpm,input.arrangement.timeSignature,input.arrangement.durationSeconds).map(x=>({atSeconds:x.timeSeconds,accent:x.beat===1}))},cues,repeatCuePath:repeat,pad:{key:input.arrangement.selectedKey,audioPath:pad}},arrangement:{id:input.arrangement.id,name:input.arrangement.name,sourceType:input.arrangement.sourceType,sourceSha256:input.arrangement.sourceSha256,proPresenterMidi:input.arrangement.proPresenterMidi.map(x=>({atSeconds:x.atSeconds,status:x.status,data1:x.data1,data2:x.data2})),midiOutputName:input.midiOutputName??null}}]};
...
async function buildCue(directory:string,label:string,output:string,ffmpeg:string){const normalized=label.replace(/([A-Za-z])([0-9])$/,"$1 $2").replace(/^Turnaround/i,"Turn Around"),aliases:Record<string,string>={START:"CountIn.wav"},direct=[aliases[normalized.toUpperCase()]??`${normalized.toUpperCase()}.wav`,`${normalized.toUpperCase().replace(/\s+/g,"")}.wav`];for(const name of direct){const candidate=join(directory,name);if(await exists(candidate)){await copyFile(candidate,output);return;}}const match=normalized.match(/^(.*)\s+([1-6])$/);if(!match)throw new Error(`No cue audio for ${label}`);const numbers=["ZERO","ONE","TWO","THREE","FOUR","FIVE","SIX"],base=join(directory,`${match[1]!.toUpperCase()}.wav`),number=join(directory,`${numbers[Number(match[2])]}.wav`);if(!(await exists(base))||!(await exists(number)))throw new Error(`No cue components for ${label}`);await run(ffmpeg,["-hide_banner","-loglevel","error","-y","-i",base,"-i",number,"-filter_complex","[0:a][1:a]concat=n=2:v=0:a=1[out]","-map","[out]","-c:a","pcm_s16le",output]);}
