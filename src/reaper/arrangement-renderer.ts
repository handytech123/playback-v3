...
export async function renderArrangementTracks(arrangement:ArrangementVersion,outputDirectory:string,ffmpegPath="ffmpeg"):Promise<readonly RenderedArrangementStem[]>{
  ...
  const stems:RenderedArrangementStem[]=[];for(const[trackName,items]of groups){const sorted=[...items].sort((a,b)=>a.positionSeconds-b.positionSeconds),single=sorted.length===1?sorted[0]:null;if(single&&near(single.positionSeconds,0)&&near(single.lengthSeconds,arrangement.durationSeconds)&&near(single.sourceOffsetSeconds,0)&&near(single.playRate,1)){stems.push({role:trackName,sourcePath:single.sourcePath!,durationSeconds:arrangement.durationSeconds,rendered:false});continue;}
