...
import { DEFAULT_INSTRUMENT_OUTPUTS, INSTRUMENT_OUTPUTS, PLAYBACK_OUTPUT_PROFILE } from "../audio/output-layout.js";
...
<dialog id="remoteSettings" class="settings-window"><header><div><span class="eyebrow">PLAYBACK V3</span><h2>Settings</h2><p id="settingsStatus">Configure the production system without crowding the performance surface.</p></div><button id="closeRemoteSettings">CLOSE</button></header><nav class="settings-tabs" aria-label="Settings sections"><button class="active" data-settings-tab="general">GENERAL</button><button data-settings-tab="audio">AUDIO</button><button data-settings-tab="midi">MIDI / SLIDES</button><button data-settings-tab="import">IMPORT / SYNC</button><button data-settings-tab="control">REMOTE / CONTROL</button><button data-settings-tab="system">SYSTEM CHECK</button></nav><div class="settings-pages">
  ...
  <section class="settings-page" data-settings-page="audio"><div class="settings-section-heading"><span>AUDIO ENGINE</span><h3>Device, Dante routing, and output health</h3></div><div class="settings-grid"><label>Audio Device<select id="audioSelect" title="Audio output"></select></label><label>Active Outputs<select id="audioOutputCount" title="Number of active output channels"></select></label><div class="settings-readout"><span>Routing</span><strong id="routeStatus"></strong></div><div class="settings-readout"><span>IEM Outputs</span><strong id="settingsIemStatus">—</strong></div></div><div class="settings-subheading"><span>DANTE / OUTPUT BUSES</span><strong>INSTRUMENTS MONO · TRACKS STEREO</strong></div><div id="danteInstrumentRoutes" class="dante-routing-grid"></div><div class="dante-routing-grid dante-aux-routes"><label data-route-width="1"><span>CLICK <b>MONO</b></span><select id="danteClickOutput"></select></label><label data-route-width="1"><span>CUE <b>MONO</b></span><select id="danteCueOutput"></select></label><label data-route-width="2"><span>PAD <b>STEREO</b></span><select id="dantePadOutput"></select></label><label data-route-width="2"><span>IEM SEND <b>STEREO</b></span><select id="danteIemOutput"></select></label></div><div class="settings-grid"><button id="applyDanteRouting" class="primary">APPLY ROUTING + RE-ARM</button><div id="danteRoutingStatus" class="settings-report">Routing changes are saved locally and applied to the native engine.</div><button id="refreshAudioSettings">REFRESH DEVICE STATUS</button><button id="runAudioCheck">RUN AUDIO ERROR CHECK</button><div id="audioCheckReport" class="settings-report settings-wide">Select Run Audio Error Check for a live readiness report.</div></div></section>
  ...
  <section class="settings-page" data-settings-page="import"><div class="settings-section-heading"><span>IMPORT / REFRESH</span><h3>Reaper arrangements and library synchronization</h3></div><div class="settings-action-list"><button id="importReaper"><strong>IMPORT REAPER ARRANGEMENT</strong><small>Preview regions, tempo/key changes, and Slides MIDI before writing.</small></button><button id="settingsRefreshLibrary"><strong>REFRESH PREPARED LIBRARY</strong><small>Reload prepared Original Songs and arrangements available to the set builder.</small></button><button id="settingsScanLibrary"><strong>SCAN MASTER LIBRARY</strong><small>Check master spreadsheet entries, song folders, and analyzer readiness.</small></button></div><div id="settingsSyncStatus" class="settings-report">No import or synchronization task is running.</div></section>
  <section class="settings-page" data-settings-page="control"><div class="settings-section-heading"><span>STAGE CONTROL</span><h3>Remote, OSC, MIDI input, and console</h3></div><div class="remote-settings"><p id="remoteStatus"></p><label>Remote URL · private<input id="remoteUrl" type="password" readonly></label><div class="remote-buttons"><button id="copyRemoteUrl">COPY PRIVATE LINK</button><button id="toggleLanRemote">ENABLE LAN</button><button id="toggleOsc">OSC ON</button></div><dl><div><dt>HTTP</dt><dd id="remoteHttp">—</dd></div><div><dt>OSC</dt><dd id="remoteOsc">—</dd></div></dl><section class="midi-input-settings"><h3>Foot Controller / MIDI Input</h3><div><label>Input<select id="midiInputDevice"></select></label><label>Profile<select id="footControllerProfile"><option value="disabled">Disabled</option><option value="basic-notes">Basic Notes · CH 1 · 20–26</option></select></label><button id="applyMidiInput">APPLY + ARM</button></div><p id="midiInputStatus">MIDI input is disabled.</p></section><section class="midi-input-settings"><h3>Allen &amp; Heath GLD-112 · Safe Test / Learn</h3><div><label>Console IP<input id="gldHost" placeholder="192.168.1.60"></label><label>MIDI Channel<input id="gldChannel" type="number" min="1" max="16" value="1"></label><button id="testGld">TEST CONNECTION</button></div><div class="gld-preview"><button id="previewGld">PREVIEW INPUT 1 MUTE</button><code id="gldHex">Writes locked</code></div><p id="gldStatus">No console writes are permitted before physical acceptance.</p></section><p class="remote-warning">LAN access requires the private token in the link. OSC on LAN also requires the token as the first OSC argument. Audio continues if a control adapter fails.</p></div></section>
...
function setupDeviceSelectors() {
  ...
  const professional=document.createElement("optgroup"),other=document.createElement("optgroup");professional.label="Professional / multichannel";other.label="Other Windows devices";
  for (const device of [...data.audio.devices].sort((a:any,b:any)=>Number(/asio|dante/i.test(`${b.type} ${b.name}`))-Number(/asio|dante/i.test(`${a.type} ${a.name}`))||a.name.localeCompare(b.name))) {
    const selected = data.audio.selectedDevice?.type === device.type && data.audio.selectedDevice?.name === device.name;
    const option=new Option(`${device.name} · ${device.type}`, JSON.stringify(device), false, selected);(/asio|dante/i.test(`${device.type} ${device.name}`)?professional:other).append(option);
  }
  if(professional.children.length)audio.append(professional);if(other.children.length)audio.append(other);
  const outputCount=$<HTMLSelectElement>("#audioOutputCount"),renderOutputCounts=(device:any)=>{const max=Math.max(2,Number(device?.maxOutputChannels??device?.outputChannels??data.audio.outputChannels??2)),selected=Math.min(max,Number(device?.outputChannels??data.audio.selectedDevice?.outputChannels??max));outputCount.replaceChildren();const choices=[...new Set([2,4,6,8,16,32,64,128,max].filter(value=>value<=max))].sort((a,b)=>a-b);for(const count of choices)outputCount.add(new Option(count===max?`${count} · ALL AVAILABLE`:String(count),String(count),false,count===selected));outputCount.disabled=!device;};
  ...
  audio.onchange = async () => {
    ...
    try { const choice=audio.value?JSON.parse(audio.value):null,device=choice?{...choice,outputChannels:choice.maxOutputChannels}:null;const state = await window.playback.audio.setDevice(device); data.audio={...data.audio,...state};renderOutputCounts(device);setRouteStatus(state.routingReady, state.outputChannels);setupDanteRouting();renderDawMixer(); }
    catch (error) { route.className = "route-status fault"; route.textContent = "AUDIO FAULT"; showError(error); }
    finally { audio.disabled = false; }
  };
  outputCount.onchange=async()=>{if(!audio.value)return;outputCount.disabled=true;route.textContent="AUDIO ARMING";try{const device={...JSON.parse(audio.value),outputChannels:Number(outputCount.value)};const state=await window.playback.audio.setDevice(device);data.audio={...data.audio,...state};setRouteStatus(state.routingReady,state.outputChannels);setupDanteRouting();renderDawMixer();}catch(error){route.className="route-status fault";route.textContent="AUDIO FAULT";showError(error);}finally{outputCount.disabled=false;}};
  setupDanteRouting();
  ...
  function setRouteStatus(ready: boolean, channels: number) { route.className = `route-status ${ready ? "ready" : "fallback"}`; route.textContent = ready ? "16 OUT READY" : `${channels} OUT FALLBACK`; }
...
function stemLabelFromPath(path:string):string{const name=String(path??"").split(/[\\/]/).pop()??"Tracks";return name.replace(/\.[^.]+$/g,"").replace(/[_-]+/g," ").replace(/\bglued\b/gi,"").trim()||"Tracks";}
function setupDanteRouting(){
  const routing=data.audio.routing??{profile:PLAYBACK_OUTPUT_PROFILE,music:{firstOutput:15,channels:1},click:{firstOutput:1,channels:1},cue:{firstOutput:2,channels:1},pad:{firstOutput:16,channels:1},iemFirstOutput:3,instrumentOutputs:DEFAULT_INSTRUMENT_OUTPUTS};
  const addOptions=(select:HTMLSelectElement,width:1|2,selected:number)=>{select.replaceChildren();for(let output=1;output<=128-(width-1);++output)select.add(new Option(width===2?`Dante ${output}–${output+1}`:`Dante ${output}`,String(output),false,output===selected));};
  const container=$("#danteInstrumentRoutes");container.replaceChildren();
  for(const bus of INSTRUMENT_OUTPUTS){const label=document.createElement("label");label.dataset.instrumentRoute=bus.key;label.dataset.routeWidth="1";label.innerHTML=`<span>${String(bus.output).padStart(2,"0")} · ${escapeHtml(bus.appBus.toUpperCase())} · ${escapeHtml(bus.danteLabel)} <b>MONO</b><small>${escapeHtml(bus.destination)}</small></span><select aria-label="${escapeHtml(bus.appBus)} output"></select>`;addOptions(label.querySelector("select")!,1,Number(routing.instrumentOutputs?.[bus.key]??bus.output));container.append(label);}
  addOptions($<HTMLSelectElement>("#danteClickOutput"),1,Number(routing.click.firstOutput));addOptions($<HTMLSelectElement>("#danteCueOutput"),1,Number(routing.cue.firstOutput));addOptions($<HTMLSelectElement>("#dantePadOutput"),1,Number(routing.pad.firstOutput));addOptions($<HTMLSelectElement>("#danteIemOutput"),2,Number(routing.iemFirstOutput));
  const auxiliaryRoutes=document.querySelector<HTMLElement>(".dante-aux-routes")!,routeLabels=[...auxiliaryRoutes.querySelectorAll<HTMLElement>("label")],clickLabel=routeLabels.find(label=>label.querySelector("#danteClickOutput")),cueLabel=routeLabels.find(label=>label.querySelector("#danteCueOutput")),padLabel=routeLabels.find(label=>label.querySelector("#dantePadOutput")),iemLabel=routeLabels.find(label=>label.querySelector("#danteIemOutput"));
  if(clickLabel)clickLabel.querySelector("span")!.innerHTML="01 · DYNAMIC CLICK · PB_CLICK <b>MONO</b><small>GLD Input 30</small>";if(cueLabel)cueLabel.querySelector("span")!.innerHTML="02 · DYNAMIC CUE · PB_CUE <b>MONO</b><small>GLD Input 31</small>";if(iemLabel){iemLabel.querySelector("span")!.innerHTML="03–04 · IEM SEND · PB_IEM_L / PB_IEM_R <b>2× MONO</b><small>Direct IEM destination</small>";auxiliaryRoutes.insertBefore(iemLabel,padLabel??null);}if(padLabel){padLabel.dataset.routeWidth="1";padLabel.querySelector("span")!.innerHTML="16 · PLAYBACK PAD · PB_PAD <b>MONO</b><small>GLD Input 44</small>";}
  const routingHeading=document.querySelector<HTMLElement>("[data-settings-page='audio'] .settings-subheading strong");if(routingHeading)routingHeading.textContent="FIXED 16-OUTPUT PROFILE · ALL PROGRAM BUSES MONO";
  const danteInstalled=(data.audio.devices??[]).some((device:any)=>/dante/i.test(`${device.type} ${device.name}`)),danteActive=/dante/i.test(`${data.audio.selectedDevice?.type??""} ${data.audio.selectedDevice?.name??""}`);for(const control of [...container.querySelectorAll<HTMLSelectElement>("select"),$<HTMLSelectElement>("#danteClickOutput"),$<HTMLSelectElement>("#danteCueOutput"),$<HTMLSelectElement>("#dantePadOutput"),$<HTMLSelectElement>("#danteIemOutput")])control.disabled=!danteActive;$<HTMLButtonElement>("#applyDanteRouting").disabled=!danteActive;container.classList.toggle("locked",!danteActive);document.querySelector(".dante-aux-routes")?.classList.toggle("locked",!danteActive);$("#danteRoutingStatus").textContent=danteActive?"DANTE DEVICE ACTIVE · routing changes will re-arm the native engine.":danteInstalled?"DANTE DETECTED · select the Dante device above to unlock routing.":"DANTE DEVICE NOT DETECTED · routing controls are locked.";
  $("#applyDanteRouting").onclick=async()=>{const button=$<HTMLButtonElement>("#applyDanteRouting"),status=$("#danteRoutingStatus"),instrumentOutputs:Record<string,number>={};for(const label of container.querySelectorAll<HTMLElement>("[data-instrument-route]"))instrumentOutputs[label.dataset.instrumentRoute!]=Number(label.querySelector("select")!.value);const next={music:routing.music,click:{firstOutput:Number(($<HTMLSelectElement>("#danteClickOutput")).value),channels:1},cue:{firstOutput:Number(($<HTMLSelectElement>("#danteCueOutput")).value),channels:1},pad:{firstOutput:Number(($<HTMLSelectElement>("#dantePadOutput")).value),channels:2},iemFirstOutput:Number(($<HTMLSelectElement>("#danteIemOutput")).value),instrumentOutputs};button.disabled=true;status.textContent="RE-ARMING NATIVE ENGINE…";try{const state=await window.playback.audio.setRouting(next);data.audio={...data.audio,...state,routing:next};liveState=state.performance??liveState;status.textContent=`ROUTING APPLIED · ${state.outputChannels} OUTPUTS OPEN`;$("#settingsIemStatus").textContent=state.iemReady?`OUTPUTS ${next.iemFirstOutput}–${next.iemFirstOutput+1} READY`:"IEM OUTPUTS UNAVAILABLE";renderDawMixer();}catch(error){status.textContent=`ROUTING NOT APPLIED · ${error instanceof Error?error.message:String(error)}`;showError(error);}finally{button.disabled=false;}};
}
...
function setupRemoteControl() {
  ...
  const renderSettingsSummary = () => {
    ...
    $("#settingsIemStatus").textContent = data.audio.iemReady ? "OUTPUTS 3–4 READY" : `${data.audio.outputChannels ?? 0} OUTPUT DEVICE`;
  ...
  $("#refreshAudioSettings").onclick = () => { renderSettingsSummary(); $("#settingsStatus").textContent = `Audio status refreshed · ${data.audio.outputChannels ?? 0} outputs detected.`; };
  ...
  $("#settingsScanLibrary").onclick = async () => { const button = $<HTMLButtonElement>("#settingsScanLibrary"); button.disabled = true; $("#settingsSyncStatus").textContent = "Scanning master spreadsheet and song folders…"; try { catalogState = await window.playback.prep.scan(); $("#settingsSyncStatus").textContent = `Scan complete · ${catalogState.counts.ready} ready · ${catalogState.counts["needs-analysis"]} need analysis · ${catalogState.counts["missing-folder"]} missing folders.`; } catch (error) { $("#settingsSyncStatus").textContent = `Scan failed · ${error instanceof Error ? error.message : String(error)}`; } finally { button.disabled = false; } };
  ...
  $("#testGld").onclick=async()=>{const button=$("#testGld") as HTMLButtonElement;button.disabled=true;$("#gldStatus").textContent="TESTING CONNECTION · sending no data…";try{const result=await window.playback.control.gldTest({host:($("#gldHost") as HTMLInputElement).value,midiChannel:Number(($("#gldChannel") as HTMLInputElement).value)});$("#gldStatus").textContent=result.status==="connection-tested"?"CONNECTION FOUND · writes still locked":"CONNECTION NOT READY";}catch(error){$("#gldStatus").textContent="CONNECTION FAILED · no data sent";showError(error);}finally{button.disabled=false;}};
}
...
function renderEditorTimeline() {
  ...
  for (const [index, stem] of workspace.waveforms.stems.entries()) {
    ...
    const label = document.createElement("label"); label.innerHTML = `<i class="stem-identifier" style="--stem-color:${color}"></i><strong>${escapeHtml(stem.role)}</strong>`; labels.append(label);
...
function renderDawMixer(){
  ...
  const iemReady=Boolean(data.audio.iemReady);$("#mixerIemStatus").textContent=iemReady?"IEM SEND · OUTPUTS 3–4 READY":`IEM SEND ARMED · ${data.audio.outputChannels??0} OUTPUT DEVICE`;$("#performanceMixer").classList.toggle("iem-unavailable",!iemReady);
}
