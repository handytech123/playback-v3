...
let pendingLibraryInsertIndex:number|null=null;
const pendingEditorWaveforms=new Map<string,any>();
let loadedEditorManifestPath:string|null=null;
...
function renderSetStrip(strip:HTMLElement){
  ...
  for(let index=0;index<10;index++){
    ...
    if(setSong){
      const button=document.createElement("button");button.className=`set-song-card ${setSong.mediaOnly?"media-card":""} ${index===activeIndex?"active":""}`;
...
function renderEditorSetBuilder() {
  ...
  for (let index = 0; index < 10; index += 1) {
    ...
    if (item) {
      ...
      card.draggable=true;
      card.onclick = async () => { selectedSetItemId = item.itemId;if(isMedia){clearEditorSelectionState();renderEditorSetBuilder();return;}renderEditorSetBuilder();await loadEditorItem(item.itemId); };
...
function renderSelectedSetSongVersions(items: any[]) {
  ...
  if (!item) return;
  if(item.kind==="media"){ $("#selectedSetSong").textContent = `${item.title} · Audio File`; $("#editorSelectedArrangementName").textContent = "Media File"; return; }
...
async function showSongLibraryPicker(index?:number) {
...
function setupRemoteControl() {
  ...
  $("#remoteControl").onclick = async () => {
    ...
    try { control = await window.playback.control.get(); $("#remoteStatus").textContent = control.address ? control.lanEnabled ? "HTTP REMOTE READY ON LAN" : "HTTP REMOTE READY · THIS COMPUTER ONLY" : "REMOTE ADAPTER FAULT"; $("#remoteOsc").textContent = control.oscEnabled && control.address?.oscPort ? `Port ${control.address.oscPort}${control.lanEnabled ? " · token first argument" : " · localhost"}` : "Disabled"; $("#toggleOsc").textContent = control.oscEnabled ? "OSC ON" : "OSC OFF"; $("#toggleOsc").classList.toggle("active", control.oscEnabled);const api=control.proPresenterApi??{};($<HTMLInputElement>("#proPresenterHost")).value=api.host??"127.0.0.1";($<HTMLInputElement>("#proPresenterPort")).value=String(api.port??51365);($<HTMLInputElement>("#proPresenterPlaylistName")).value=api.playlistName??"Worship";($<HTMLInputElement>("#proPresenterPlaylistId")).value=api.playlistId??"";($<HTMLSelectElement>("#proPresenterApiEnabled")).value=api.enabled===false?"false":"true";$("#proPresenterApiStatus").textContent=api.enabled===false?"API OFF":`API ON · ${api.playlistName??"Worship"} playlist · media skipped`;const input=$("#midiInputDevice") as HTMLSelectElement;input.replaceChildren(new Option("MIDI Input Disabled",""),...control.midiInput.devices.map((name:string)=>new Option(name,name,false,name===control.midiInput.selected)));($("#footControllerProfile") as HTMLSelectElement).value=control.midiInput.profile;$("#midiInputStatus").textContent=control.midiInput.enabled?`ARMED · ${control.midiInput.selected}`:"MIDI input is disabled.";const gldOutput=$("#gldMidiOutput") as HTMLSelectElement;gldOutput.replaceChildren(new Option("GLD MIDI Output Disabled",""),...control.gld.devices.map((name:string)=>new Option(name,name,false,name===control.gld.midiOutputName)));($("#gldChannel") as HTMLInputElement).value=String(control.gld.midiChannel);renderSettingsSummary();await renderHttpConnection(control).catch(()=>{$("#httpQrStatus").textContent="Remote link is ready, but the QR code could not be drawn.";});await renderOscConnection(control).catch(()=>{$("#oscQrStatus").textContent="OSC is ready, but the QR code could not be drawn.";}); } catch (error) { showError(error); }
  };
  ...
  $("#saveProPresenterApi").onclick=async()=>{const button=$<HTMLButtonElement>("#saveProPresenterApi");button.disabled=true;$("#proPresenterApiStatus").textContent="SAVING PROPRESENTER API…";try{await window.playback.control.setSettings({proPresenterApi:{enabled:($<HTMLSelectElement>("#proPresenterApiEnabled")).value==="true",host:($<HTMLInputElement>("#proPresenterHost")).value,port:Number(($<HTMLInputElement>("#proPresenterPort")).value),playlistName:($<HTMLInputElement>("#proPresenterPlaylistName")).value||"Worship",playlistId:($<HTMLInputElement>("#proPresenterPlaylistId")).value||null}});control=await window.playback.control.get();$("#proPresenterApiStatus").textContent=control.proPresenterApi.enabled?`API ON · ${control.proPresenterApi.host}:${control.proPresenterApi.port} · ${control.proPresenterApi.playlistName}`:"API OFF";}catch(error){showError(error);$("#proPresenterApiStatus").textContent="PROPRESENTER API SETTINGS FAILED";}finally{button.disabled=false;}};
...
function setupEditorControls() {
  ...
  $("#closeSongLibrary").onclick = () => { pendingLibraryInsertIndex=null;($<HTMLDialogElement>("#songLibraryPicker")).close(); };
...
function confirmedSetMatchesDraft(){
  ...
  for(let index=0;index<items.length;index+=1){
    ...
    if(String(prepared?.song?.id)!==String(item.songId))return false;
    if(item.kind==="media"){if(prepared?.mediaOnly!==true)return false;}
    else if((prepared?.arrangement?.name??"Original Song")!==item.arrangement)return false;
...
function setupPrep() {
  ...
  window.playback.prep.onWaveformsReady((state)=>{const key=`${state.itemId}|${state.manifestPath}`;pendingEditorWaveforms.set(key,state.waveforms);if(selectedSetItemId===state.itemId&&loadedEditorManifestPath===state.manifestPath&&workspace){workspace={...workspace,waveforms:state.waveforms};pendingEditorWaveforms.delete(key);requestAnimationFrame(()=>renderEditorTimeline());}});
...
async function sendSetToProPresenter(){
  const button=$("#editorSendProPresenter") as HTMLButtonElement;
  button.disabled=true;
  ...
  }catch(error){showError(error);setSetlistStatus("Send to ProPresenter failed.");}
  finally{button.disabled=false;}
}
async function addMediaToSetlist(index?:number){
  const button=$<HTMLButtonElement>("#songLibraryAddMedia");
  button.disabled=true;
  ...
  try{
    ...
    if(result?.cancelled){pendingLibraryInsertIndex=null;setSetlistStatus("Add WAV cancelled.");return;}
    prepState=result;
    ...
    if($<HTMLDialogElement>("#songLibraryPicker").open)($<HTMLDialogElement>("#songLibraryPicker")).close();
    clearEditorSelectionState();
    renderPrep();
    renderEditorSetBuilder();
    setSetlistStatus("WAV media added. Confirm Set will cache it as full audio.");
  }catch(error){showError(error);setSetlistStatus("Add WAV failed.");}
  finally{button.disabled=false;}
}
...
async function importSetlist(){
  ...
  try{
    ...
    if(selectedSetItemId&&selected?.kind!=="media")await loadEditorItem(selectedSetItemId);
...
function loadEditorItem(itemId:string){const request=editorLoadSerial.then(async()=>{if(selectedSetItemId!==itemId)return;try{loadedEditorManifestPath=null;const result=await window.playback.prep.loadItem(itemId);if(selectedSetItemId!==itemId)return;prepState=await window.playback.prep.get();loadedEditorManifestPath=result.manifestPath;workspace=result.workspace;const waveformKey=`${itemId}|${result.manifestPath}`,preparedWaveforms=pendingEditorWaveforms.get(waveformKey);if(preparedWaveforms){workspace={...workspace,waveforms:preparedWaveforms};pendingEditorWaveforms.delete(waveformKey);}selectedRegionId=workspace.draft.sections[0]?.id??null;selectionStart=null;selectionEnd=null;currentPosition=0;renderEditorSetBuilder();renderEditor();requestAnimationFrame(()=>{if(selectedSetItemId===itemId&&loadedEditorManifestPath===result.manifestPath)renderEditorTimeline();});}catch(error){if(selectedSetItemId===itemId){loadingSetItemId=null;loadingProgress=0;loadingLabel="";renderEditorSetBuilder();}throw error;}});editorLoadSerial=request.catch(()=>{});return request;}
