...
function setupPrep() {
  ...
  $("#confirmSet").onclick = async () => { const button = $("#confirmSet") as HTMLButtonElement; button.disabled = true; $("#setlistStatus").textContent = "Copying and validating the isolated performance package..."; try { const result = await withGlobalLoading("CONFIRMING SET","Copying and validating the isolated performance package.",()=>window.playback.prep.confirm()); applyBootstrapPayload(result.bootstrap); localStorage.setItem("playback.ui.mode","performance"); $("#setlistStatus").textContent = `Confirmed ${result.songs} song${result.songs === 1 ? "" : "s"}. Performance loaded.`; await setMode(false); } catch (error) { localStorage.setItem("playback.ui.mode","edit"); showError(error); button.disabled = false; $("#confirmSetProgress").classList.add("fault"); $("#confirmSetProgressLabel").textContent = error instanceof Error ? error.message : String(error); $("#setlistStatus").textContent = "Confirm Set failed. Draft was preserved."; } };
}
