...
<main>
  ...
  <section id="performanceMixer" class="daw-mixer" aria-label="Live mixer"><div id="mixerResizeHandle" class="mixer-resize-handle" title="Drag to resize mixer"></div><header><div><span>LIVE MIXER</span><strong id="mixerIemStatus">IEM SEND CHECKING</strong></div><div><button id="savePerformanceMix">SAVE SONG MIX</button><button id="mixerCollapse" aria-expanded="true">COLLAPSE</button></div></header><div id="mixerChannels" class="mixer-channels"></div></section>
</main>
...
const mixerViewportSelectors=["#editorTimelineScroll",".summary-stem-channel-row","#mixerChannels","#performanceMixer"] as const;
type MixerViewportSnapshot={readonly windowX:number;readonly windowY:number;readonly scrolls:readonly{selector:string;left:number;top:number}[]};
function captureMixerViewport():MixerViewportSnapshot{return{windowX:window.scrollX,windowY:window.scrollY,scrolls:mixerViewportSelectors.flatMap(selector=>{const element=document.querySelector<HTMLElement>(selector);return element?[{selector,left:element.scrollLeft,top:element.scrollTop}]:[]})};}
function restoreMixerViewport(snapshot:MixerViewportSnapshot){const restore=()=>{window.scrollTo(snapshot.windowX,snapshot.windowY);for(const saved of snapshot.scrolls){const element=document.querySelector<HTMLElement>(saved.selector);if(element){element.scrollLeft=saved.left;element.scrollTop=saved.top;}}};restore();requestAnimationFrame(restore);}
async function preserveMixerViewport<T>(task:()=>Promise<T>):Promise<T>{const snapshot=captureMixerViewport();if(document.activeElement instanceof HTMLElement)document.activeElement.blur();try{return await task();}finally{restoreMixerViewport(snapshot);}}
...
function setupPerformance() {
  ...
  $("#savePerformanceMix").onclick=async()=>{const button=$("#savePerformanceMix") as HTMLButtonElement,original=button.textContent;button.disabled=true;button.textContent="SAVING…";try{const saved=await window.playback.performance.saveMix();button.textContent=`SAVED · ${saved.title}`;window.setTimeout(()=>button.textContent=original,2200);}catch(error){button.textContent="SAVE FAILED";showError(error);window.setTimeout(()=>button.textContent=original,2200);}finally{button.disabled=false;}};
  setupMixerResize();
...
function renderEditorTimeline() {
  ...
  for (const [displayIndex, entry] of editorStemDisplayOrder().entries()) {
    ...
    for(const button of label.querySelectorAll<HTMLButtonElement>("button"))button.onclick=async()=>{try{await preserveMixerViewport(async()=>{const latest=workspace.mixer.channels[index];await updateEditorStemMix(index,{muted:button.dataset.stemSwitch==="muted"?!latest.muted:latest.muted,solo:button.dataset.stemSwitch==="solo"?!latest.solo:latest.solo});renderEditorTimeline();});}catch(error){showError(error);}};
...
function renderPerformanceReadiness(report: any) {
  ...
  for (const control of document.querySelectorAll<HTMLInputElement | HTMLButtonElement>("#liveControls button, #liveControls input, #performanceMixer button, #performanceMixer input, #play, #pause, #pad, #slidesMidi, #surfaceMidi")) if (control.id !== "clearFault"&&control.id!=="mixerCollapse"&&control.id!=="savePerformanceMix") control.disabled = locked;
...
async function liveCommand(command: any) { const run=async()=>{const state=await window.playback.performance.command(command);if(state.songIndex!==activeSongIndex){await synchronizePerformanceSong(state.songIndex,state);return;}liveState=state;renderLiveState();};try{if(command.action==="mixer-channel")await preserveMixerViewport(run);else await run();}catch(error){showError(error);} }
