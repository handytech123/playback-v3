...
function instrumentBusName(label:string):string{return label.trim().replace(/\.[^.]+$/g,"").replace(/[_-]+/g," ").replace(/\s+\d+$/g,"").replace(/\s+/g," ")||"Tracks";}
function instrumentBusKey(label:string):string{return instrumentBusName(label).toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")||"tracks";}
function instrumentBusChannels(label:string):1|2{return /^tracks?$/i.test(instrumentBusName(label))?2:1;}
...
function setupDanteRouting(){
  const routing=data.audio.routing??{music:{firstOutput:1,channels:2},click:{firstOutput:3,channels:1},cue:{firstOutput:4,channels:1},pad:{firstOutput:5,channels:2},iemFirstOutput:7,instrumentOutputs:{}};
  const labels=[...(data.stemLabels??[]),...data.manifest.songs.flatMap((item:any)=>item.stems?.map((stem:any)=>stemLabelFromPath(stem.sourcePath))??[])];
  const buses=[...new Map(labels.map((label:string)=>[instrumentBusKey(label),instrumentBusName(label)])).entries()].sort((a,b)=>a[1].localeCompare(b[1]));
  ...
  for(const[key,name]of buses){const width=instrumentBusChannels(name),label=document.createElement("label");label.dataset.instrumentRoute=key;label.dataset.routeWidth=String(width);label.innerHTML=`<span>${escapeHtml(name.toUpperCase())} <b>${width===2?"STEREO":"MONO"}</b></span><select aria-label="${escapeHtml(name)} output"></select>`;addOptions(label.querySelector("select")!,width,Number(routing.instrumentOutputs?.[key]??routing.music.firstOutput));container.append(label);}
  addOptions($<HTMLSelectElement>("#danteClickOutput"),1,Number(routing.click.firstOutput));addOptions($<HTMLSelectElement>("#danteCueOutput"),1,Number(routing.cue.firstOutput));addOptions($<HTMLSelectElement>("#dantePadOutput"),2,Number(routing.pad.firstOutput));addOptions($<HTMLSelectElement>("#danteIemOutput"),2,Number(routing.iemFirstOutput));
