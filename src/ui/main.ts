...
<dialog id="remoteSettings" class="settings-window"><header><div><span class="eyebrow">PLAYBACK V3</span><h2>Settings</h2><p id="settingsStatus">Configure the production system without crowding the performance surface.</p></div><button id="closeRemoteSettings">CLOSE</button></header><nav class="settings-tabs" aria-label="Settings sections"><button class="active" data-settings-tab="audio">AUDIO</button><button data-settings-tab="import">LIBRARY / ANALYSIS</button><button data-settings-tab="control">REMOTE / CONTROL</button><button data-settings-tab="transitions">TRANSITIONS</button><button data-settings-tab="midi">MIDI</button><button data-settings-tab="system">SYSTEM CHECK</button></nav><div class="settings-pages">
  ...
  <section class="settings-page" data-settings-page="control"><div class="settings-section-heading"><span>STAGE CONTROL</span><h3>Remote, OSC, and MIDI input</h3></div><div class="remote-settings"><p id="remoteStatus"></p><label>Remote URL · private<input id="remoteUrl" type="password" readonly></label><div class="remote-buttons"><button id="copyRemoteUrl">COPY PRIVATE LINK</button><button id="toggleLanRemote">ENABLE LAN</button><button id="toggleOsc">OSC ON</button></div><dl><div><dt>HTTP</dt><dd id="remoteHttp">—</dd></div><div><dt>OSC</dt><dd id="remoteOsc">—</dd></div></dl><section id="oscQrCard" class="osc-qr-card offline"><div class="osc-qr-copy"><span>OSC REMOTE CONNECTION</span><h3>Scan To Connect</h3><p id="oscQrStatus">Enable LAN and OSC to create a stage-ready connection code.</p><label>Network Address<select id="oscQrAddress"></select></label><label>Private OSC Profile<input id="oscQrPayload" type="password" readonly></label><button id="copyOscProfile">COPY OSC PROFILE</button></div><div class="osc-qr-code"><canvas id="oscQrCanvas" width="210" height="210"></canvas><small>PRIVATE · DO NOT SHARE PUBLICLY</small></div></section><section class="midi-input-settings"><h3>Foot Controller / MIDI Input</h3><div><label>Input<select id="midiInputDevice"></select></label><label>Profile<select id="footControllerProfile"><option value="disabled">Disabled</option><option value="basic-notes">Basic Notes · CH 1 · 20–26</option></select></label><button id="applyMidiInput">APPLY + ARM</button></div><p id="midiInputStatus">MIDI input is disabled.</p></section><p class="remote-warning">LAN access requires the private token in the link. OSC on LAN also requires the token as the first OSC argument. Audio continues if a control adapter fails.</p></div></section>
...
function setupWindowsMenu(){
  ...
  bridge?.onMenuAction((action:string)=>{
    const selectors:Record<string,string>={settings:"#remoteControl","import-reaper":"#importReaper",play:"#play",pause:"#pause",stop:"#stop",panic:"#panic","performance-mode":"#performanceMode","edit-mode":"#editMode"};
...
function setupDeviceSelectors() {
  ...
  const renderOutputMatrix=()=>{
    ...
    const instrumentBuses=PLAYBACK_OUTPUTS.filter((bus)=>bus.output>=5&&bus.output<=15).map((bus)=>{const indices=data.audio.routingLabels.map((label:string,index:number)=>({index,key:classifyStemOutput(label)})).filter((item:any)=>item.key===bus.key).map((item:any)=>item.index),outputs=indices.map((index:number)=>data.audio.routing.stems[index]),widths=indices.map((index:number)=>data.audio.routing.stemChannels[index]),sameOutput=outputs.length>0&&outputs.every((value:number)=>value===outputs[0]),sameWidth=widths.length>0&&widths.every((value:number)=>value===widths[0]);return{label:bus.appBus,width:sameWidth?widths[0]:1,widthKey:"stemChannels",kind:"stem-bus",indices,value:sameOutput?outputs[0]:0,mixed:!sameOutput||!sameWidth};}).filter((bus)=>bus.indices.length>0);
    const fields:any[]=[{label:"Dynamic Click",width:data.audio.routing.clickChannels,widthKey:"clickChannels",kind:"click",value:data.audio.routing.click},{label:"Dynamic Cue",width:data.audio.routing.cueChannels,widthKey:"cueChannels",kind:"cue",value:data.audio.routing.cue},{label:"IEM Send L/R",width:data.audio.routing.iemChannels,widthKey:"iemChannels",kind:"iem",value:data.audio.routing.iem},...instrumentBuses,{label:"Playback Pad",width:data.audio.routing.padChannels,widthKey:"padChannels",kind:"pad",value:data.audio.routing.pad}];
    ...
    for(const field of fields){
      const heading=document.createElement("span");heading.innerHTML=`<strong>${escapeHtml(field.label)}</strong><button class="route-mode">${field.mixed?"MIXED":field.width===2?(stereo?"BOTH":"STEREO"):(stereo&&field.value===2?"RIGHT":stereo?"LEFT":"MONO")}</button>`;grid.append(heading);
      const mode=heading.querySelector<HTMLButtonElement>(".route-mode");
      if(mode)mode.onclick=async()=>{const routing:any=makeRouting();let nextWidth:1|2,nextOutput:number;if(stereo){if(field.width===2&&!field.mixed){nextWidth=1;nextOutput=1;}else if(field.value===1&&!field.mixed){nextWidth=1;nextOutput=2;}else{nextWidth=2;nextOutput=1;}setRoute(routing,field,nextOutput);}else{nextWidth=field.width===2&&!field.mixed?1:2;nextOutput=field.value;}if(field.kind==="stem-bus")for(const index of field.indices)routing.stemChannels[index]=nextWidth;else routing[field.widthKey]=nextWidth;grid.classList.add("busy");try{const state=await window.playback.audio.setRouting(routing);data.audio={...data.audio,...state};$("#settingsStatus").textContent=stereo?`${field.label} output balance updated.`:`${field.label} channel mode updated.`;}catch(error){showError(error);$("#settingsStatus").textContent="Output selection could not be saved.";}renderOutputMatrix();};
...
function setupRemoteControl() {
  ...
  $("#toggleLanRemote").onclick = async () => { if (!control) return; $("#remoteStatus").textContent = "RESTARTING CONTROL ADAPTER…"; await window.playback.control.setSettings({ lanEnabled: !control.lanEnabled });control=await window.playback.control.get();await renderOscConnection(control);$("#toggleLanRemote").textContent=control.lanEnabled?"DISABLE LAN":"ENABLE LAN";$("#remoteStatus").textContent=control.lanEnabled?"LAN REMOTE READY":"LOCAL TEST MODE"; };
