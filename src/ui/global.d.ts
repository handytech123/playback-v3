...
declare global {
  interface Window {
    playback: {
      ...
      audio: { setDevice(device: { type: string; name: string } | null): Promise<any>; setRouting(routing: any): Promise<any>; refresh(): Promise<any> };
      ...
      prep: { get(): Promise<any>; status(): Promise<any>; scan(): Promise<any>; review(songId: string): Promise<any>; command(command: any): Promise<any>; loadItem(itemId: string): Promise<any>; confirm(options?: { selectedIndex?: number }): Promise<any>; onStatus(listener: (state: any) => void): void; onLoadStatus(listener: (state: { itemId: string; progress: number; label: string }) => void): void; onConfirmStatus(listener: (state: { progress: number; label: string }) => void): void };
