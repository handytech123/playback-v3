...
declare global {
  interface Window {
    playback: {
      ...
      performance: { get(): Promise<any>; command(value: any): Promise<any>; exportSong(options?: { itemId?: string }): Promise<any>; onState(listener: (state: any) => void): void; onMeters(listener: (meters: { master: number; channels: readonly number[] }) => void): void };
      ...
      audio: { setDevice(device: { type: string; name: string } | null): Promise<any>; setRouting(routing: any): Promise<any>; setGlobalBusRouting(routing: any): Promise<any>; setGlobalBusRoutingLock(locked:boolean):Promise<any>; refresh(): Promise<any>; onHealth(listener:(health:any)=>void):void };
      ...
      prep: { get(): Promise<any>; status(): Promise<any>; update(): Promise<any>; review(songId: string): Promise<any>; command(command: any): Promise<any>; loadItem(itemId: string): Promise<any>; confirm(options?: { selectedIndex?: number }): Promise<any>; sendToProPresenter(): Promise<any>; exportSetlist(): Promise<any>; importSetlist(): Promise<any>; onStatus(listener: (state: any) => void): void; onLoadStatus(listener: (state: { itemId: string; progress: number; label: string }) => void): void; onWaveformsReady(listener:(state:{itemId:string;manifestPath:string;waveforms:any})=>void):void; onConfirmStatus(listener: (state: { progress: number; label: string }) => void): void };
