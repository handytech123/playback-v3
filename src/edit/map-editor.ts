...
import type { MusicalPosition } from "../domain/song.js";
...
export type MapCommand =
  ...
  | { readonly type:"set-region-boundary"; readonly rightRegionId:string; readonly atPosition:MusicalPosition }
  | { readonly type:"add-region-boundary"; readonly containingRegionId:string; readonly newRegionId:string; readonly atPosition:MusicalPosition; readonly sectionType:SectionType }
  ...
  | { readonly type:"move-cue"; readonly cueId:string; readonly atPosition:MusicalPosition }
...
export function applyMapCommand(map:OriginalSongMap,command:MapCommand):OriginalSongMap {
  ...
  if(command.type==="set-region-type")regions=regions.map((region)=>region.id===command.regionId?{...region,sectionType:command.sectionType,modifier:command.modifier===undefined?region.modifier:command.modifier,approved:false}:region);
    ...
    const right=regions.findIndex((region)=>region.id===command.rightRegionId);if(right<=0)throw new Error("Boundary must have regions on both sides");const at=positionToSeconds(command.atPosition,map.bpm,map.timeSignature);if(at<=regions[right-1]!.startSeconds||at>=regions[right]!.endSeconds)throw new Error("Boundary would create an empty region");regions=regions.map((region,index)=>index===right-1?{...region,endSeconds:at,approved:false}:index===right?{...region,startSeconds:at,approved:false}:region);
  } else if(command.type==="add-region-boundary"){
    if(regions.some((region)=>region.id===command.newRegionId))throw new Error("New region ID already exists");const index=regions.findIndex((region)=>region.id===command.containingRegionId);if(index<0)throw new Error("Containing region not found");const current=regions[index]!,at=positionToSeconds(command.atPosition,map.bpm,map.timeSignature);if(at<=current.startSeconds||at>=current.endSeconds)throw new Error("New boundary is outside the selected region");regions.splice(index,1,{...current,endSeconds:at,approved:false},{id:command.newRegionId,name:"",sectionType:command.sectionType,occurrence:1,sourceLabel:"",modifier:null,approved:false,startPosition:command.atPosition,startSeconds:at,endSeconds:current.endSeconds});
  ...
  else if(command.type==="move-cue")cues=replaceCue(cues,command.cueId,(cue)=>({...cue,position:command.atPosition,atSeconds:positionToSeconds(command.atPosition,map.bpm,map.timeSignature)}));
  ...
  const locate=(seconds:number)=>secondsToMusicalPosition(seconds,map.bpm,map.timeSignature),result={...map,revision:map.revision+1,reviewState,regions:renumberRegions(regions).map(region=>({...region,startPosition:locate(region.startSeconds),endPosition:locate(region.endSeconds)})),cues:cues.map(cue=>({...cue,position:cue.position??locate(cue.atSeconds)}))};const issues=validateSongMap(result);if(issues.length)throw new Error(`Invalid map edit: ${issues.join("; ")}`);return result;
}
