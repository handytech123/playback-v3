...
export function reconcileAudioDevice(saved:NativeAudioDeviceSelection|null,available:readonly NativeAudioDeviceSelection[]):NativeAudioDeviceSelection|null{if(!saved)return null;const exact=available.find(device=>device.type===saved.type&&device.name===saved.name),fuzzy=exact??available.find(device=>normalize(device.type)===normalize(saved.type)&&normalizeDeviceName(device.name)===normalizeDeviceName(saved.name));if(!fuzzy)return null;const max=Math.max(2,Number(fuzzy.maxOutputChannels??fuzzy.outputChannels??2)),requested=Math.max(2,Number(saved.outputChannels??max));return{type:fuzzy.type,name:fuzzy.name,maxOutputChannels:max,outputChannels:Math.min(requested,max)};}
