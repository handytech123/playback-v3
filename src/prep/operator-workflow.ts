...
export async function repairOperatorSetlistForConfirmation(setlist:OperatorSetlist,repair:(item:OperatorSetlistItem,index:number)=>Promise<PreparedLibraryChoice>):Promise<OperatorSetlist>{
  let items:OperatorSetlistItem[]|null=null;
  for(const[index,item]of setlist.items.entries()){
    const repaired=await repair(item,index);
    if(repaired.manifestPath===item.manifestPath&&repaired.songIndex===item.songIndex)continue;
    items??=[...setlist.items];
    items[index]={...repaired,itemId:item.itemId,...(item.transitionToNext?{transitionToNext:item.transitionToNext}:{}),...(item.stemMix?{stemMix:item.stemMix}:{})};
  }
  return items?{...setlist,items,updatedAt:new Date().toISOString()}:setlist;
}
