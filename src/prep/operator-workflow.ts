...
export const AUDIO_MEDIA_VENDOR = "Playback Media";
...
function isAudioMediaSong(song:PreparedSong):boolean{return song.song.vendor===AUDIO_MEDIA_VENDOR||String(song.song.id).startsWith("media-");}

function autoApprovalIssues(song:PreparedSong):string[]{
  if(isAudioMediaSong(song))return [];
  const issues:string[]=[];
