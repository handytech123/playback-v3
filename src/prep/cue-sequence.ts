...
export function countedCueDelays(bpm: number, meter: TimeSignature): readonly { label: string; delaySeconds: number }[] {
...
export async function writeCountedCue(input: { sourcePath: string; destinationPath: string; numberDirectory: string; bpm: number; meter: TimeSignature; ffmpegPath?: string }): Promise<void> {
  const delays = countedCueDelays(input.bpm, input.meter);
