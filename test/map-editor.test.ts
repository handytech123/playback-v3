...
test("grid-snaps boundaries, renumbers types, and retargets cues",()=>{let map=applyMapCommand(fixture(),{type:"set-region-boundary",rightRegionId:"r2",atSeconds:4.2});assert.equal(map.regions[0]!.endSeconds,4);map=applyMapCommand(map,{type:"set-region-type",regionId:"r2",sectionType:"chorus"});assert.deepEqual(map.regions.map((region)=>region.name),["Verse","Chorus"]);map=applyMapCommand(map,{type:"retarget-cue",cueId:"c1",targetRegionId:"r1"});assert.equal(map.cues[0]!.targetRegionId,"r1");});
