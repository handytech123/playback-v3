...
test("defines the fixed 16-channel Playback output contract",()=>{
  assert.deepEqual(PLAYBACK_OUTPUTS.map(({output,danteLabel})=>[output,danteLabel]),[
    ...
    [9,"PB_ELECTRIC"],[10,"PB_PIANO"],[11,"PB_ORGAN"],[12,"PB_SYNTHS"],[13,"PB_ORCHESTRA"],[14,"PB_VOCALS"],[15,"PB_MISC"],[16,"PB_PAD"],
  ]);
  assert.deepEqual(DEFAULT_INSTRUMENT_OUTPUTS,{drums:5,percussion:6,bass:7,acoustic:8,electric:9,piano:10,organ:11,synths:12,orchestra:13,vocals:14,misc:15});
...
test("classifies common production stem labels into stable output buses",()=>{
  const cases:Record<string,string>={"DRUMS 2":"drums",SHAKER:"percussion",BASS:"bass","ACOUSTIC_1":"acoustic","ELECTRIC 3":"electric",PIANO:"piano",ORGAN:"organ",KEYS:"synths",CELLO:"orchestra",BGVS:"vocals",Pad_C:"pad",HORNS:"misc",TRUMPET:"misc","Tracks 1":"misc"};
