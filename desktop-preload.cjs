...
contextBridge.exposeInMainWorld("playback", {
  ...
  audio: { setDevice:(device)=>ipcRenderer.invoke("audio:set-device",device), setRouting:(routing)=>ipcRenderer.invoke("audio:set-routing",routing), refresh:()=>ipcRenderer.invoke("audio:refresh") },
  transitions: { get:()=>ipcRenderer.invoke("transitions:get"), set:(settings)=>ipcRenderer.invoke("transitions:set",settings) },
  prep: { get:()=>ipcRenderer.invoke("prep:get"),status:()=>ipcRenderer.invoke("prep:status"),scan:()=>ipcRenderer.invoke("prep:scan"),review:(songId)=>ipcRenderer.invoke("prep:review",songId),command:(command)=>ipcRenderer.invoke("prep:command",command),loadItem:(itemId)=>ipcRenderer.invoke("prep:load-item",itemId),confirm:(options)=>ipcRenderer.invoke("prep:confirm",options),onStatus:(listener)=>ipcRenderer.on("prep:status",(_event,state)=>listener(state)),onLoadStatus:(listener)=>ipcRenderer.on("editor:load-status",(_event,state)=>listener(state)),onConfirmStatus:(listener)=>ipcRenderer.on("prep:confirm-status",(_event,state)=>listener(state)) },
