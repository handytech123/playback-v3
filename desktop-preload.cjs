...
contextBridge.exposeInMainWorld("playback", {
  ...
  prep: { get:()=>ipcRenderer.invoke("prep:get"),status:()=>ipcRenderer.invoke("prep:status"),scan:()=>ipcRenderer.invoke("prep:scan"),command:(command)=>ipcRenderer.invoke("prep:command",command),confirm:(options)=>ipcRenderer.invoke("prep:confirm",options),onStatus:(listener)=>ipcRenderer.on("prep:status",(_event,state)=>listener(state)) },
